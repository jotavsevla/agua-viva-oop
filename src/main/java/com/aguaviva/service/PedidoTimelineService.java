...
public class PedidoTimelineService {
    ...
    public PedidoTimelineResultado consultarTimeline(int pedidoId) {
        ...
        try (Connection conn = connectionFactory.getConnection()) {
            ...
            for (EventoDispatch evento : buscarEventosPedido(conn, pedidoId)) {
                switch (evento.eventType()) {
                    case DispatchEventTypes.PEDIDO_CRIADO -> {
                        eventos.add(new TimelineEvent(evento.createdEm(), "NOVO", "PENDENTE", "Atendimento", null));
                        encontrouEventoCriacao = true;
                    }
                    case DispatchEventTypes.PEDIDO_ENTREGUE ->
                        eventos.add(new TimelineEvent(
                                evento.createdEm(), "EM_ROTA", "ENTREGUE", "Evento operacional", null));
                    case DispatchEventTypes.PEDIDO_FALHOU, DispatchEventTypes.PEDIDO_CANCELADO ->
                        eventos.add(new TimelineEvent(
                                evento.createdEm(),
                                "EM_ROTA",
                                "CANCELADO",
            ...
            if (!encontrouEventoCriacao) {
                eventos.add(new TimelineEvent(pedido.criadoEm(), "NOVO", "PENDENTE", "Atendimento", null));
            }
            ...
            if (rotaId != null) {
                ...
                if (rotaIniciada != null) {
                    eventos.add(new TimelineEvent(rotaIniciada, "CONFIRMADO", "EM_ROTA", "Despacho", null));
                }
            }

            eventos.sort(Comparator.comparing(TimelineEvent::timestamp)
                    .thenComparing(TimelineEvent::deStatus)
                    .thenComparing(TimelineEvent::paraStatus));

            List<EventoTimelineResultado> respostaEventos =
                    eventos.stream().map(PedidoTimelineService::toResponseEvent).toList();

            return new PedidoTimelineResultado(pedidoId, pedido.statusAtual(), respostaEventos);
    ...
    private PedidoAtual buscarPedido(Connection conn, int pedidoId) throws SQLException {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                ...
                return new PedidoAtual(rs.getString("status"), rs.getObject("criado_em", LocalDateTime.class));
            }
        }
    }
    ...
    private record TimelineEvent(
            LocalDateTime timestamp, String deStatus, String paraStatus, String origem, String observacao) {}
    ...
    public record EventoTimelineResultado(
            String timestamp, String deStatus, String paraStatus, String origem, String observacao) {}
}
